...
- **单文件模式** — 支持直接打开单个 Markdown 文件（文件夹功能不受影响）
...
安装包已声明 Markdown 文档关联（.md / .markdown / .mdown / .mkd）。macOS 出于安全策略不允许应用自设默认，需一次手动确认：

1. 在 Finder 中右键任意 `.md` 文件 → 显示简介（⌘I）
2. 「打开方式」下拉选择 aurora
3. 点击「全部更改…」——之后双击所有同类文件都用 Aurora 打开

也可以：右键文件 → 打开方式 → 其他… → 选择 Aurora 并勾选「始终以此方式打开」。
运行中把文件拖到 Dock 图标同样可以直接打开。
